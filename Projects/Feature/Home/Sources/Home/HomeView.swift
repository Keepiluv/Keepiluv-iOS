...
private struct HomeContentSection: View {
    ...
    #if PERF_TESTING
    /// Pass 4-S2 — guards the self-run scroll Task so it only fires once per
    /// scene appearance, even if SwiftUI invalidates the view multiple times
    /// during initial layout settling.
    @State private var selfRunScrollStarted: Bool = false
    /// Pass 4-S2 — flips to `"true"` once the self-run scrollTo sequence
    /// completes. Surfaced via `perfStateMarker` so trace analysis can isolate
    /// the post-scroll window.
    ...
    #if PERF_TESTING
    /// Pass 4-S2 — Example/perf-only branch. Wraps the same `scrollContent`
    /// in a `ScrollViewReader` (public SwiftUI API) so a self-running Task
    /// can call `proxy.scrollTo(id:anchor:)` across a stride of
    /// `home-heavy` item ids. State-driven self-run: not equivalent to
    /// real finger drag; lower bound on real scroll cost.
